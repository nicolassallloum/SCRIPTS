...
Usage examples:
  # Single file, 30M rows (default)
  python generate_tron_csv_fixed.py

  # Explicit 30M rows into one file
  python generate_tron_csv_fixed.py --rows 30000000 --out tron_data.csv

  # Sharded (8 files x 3.75M each), faster on multi-core
  python generate_tron_csv_fixed.py --rows 30000000 --shards 8 --out tron_data_part.csv

  # Gzipped shards
  python generate_tron_csv_fixed.py --rows 30000000 --shards 8 --gzip --out tron_data_part.csv.gz

  # Control starting block/timestamp
  python generate_tron_csv_fixed.py --rows 1000000 --start-block 36000000 --start '2018-01-01T00:00:00Z'
"""
import argparse, csv, gzip, os, random, sys, time
...
def main():
    ...
    ap.add_argument("--rows", type=int, default=30000000, help="Total rows to generate (default 30,000,000)")
